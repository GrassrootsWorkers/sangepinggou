...
<head>
    ...
    <link href="http://s.sangepg.com/css/personal.css?t=1" rel="stylesheet" type="text/css">
    ...
    <div class="content bg pb120">
        <form method="post" id="save_user_info" action="/admin/user/update">
            <div class="personal_top_nav">
                <ul>
                    <li class="h40">
                        <a href="http://www.sangepg.com/admin/fruit/list?pageIndex=1&farmerId=${farmer.id}"> &lt;返回个人中心</a>
                    ...
                    <li class="w540">
                        ...
                        <label>
                            ...
                            <input class="w442" id="id_card" name="idCard"  onblur="isCardNo()" placeholder="18位身份证号" value="${farmer.idCard}"/>
                        </label>
                        <label>
                            <span class="personal_span" >种植经验：</span>
                            <input class="w442" id="experience_age" name="experienceAge" value="${farmer.experienceAge}年" placeholder="多少年的种植经验"/>
                        </label>
                        <label>
                            <span class="personal_span">家庭住址：</span>
                            <input class="w442" id="home_address" name="homeTown" value="${farmer.homeTown}" placeholder="您目前所在的村的名字"/>
                        </label>
                        <%--<label>
                            <span class="personal_span">状&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;态：</span>
                            <select id="status" name="status" class="select2" style="width: 150px;font-weight: normal">
                                <option value="" >请选择</option>
                                <option value="1">未发布作业</option>
                                <option value="1">未发布作业</option>
                                <option value="2">已发布作业</option>
                                <option value="3">未发布成绩</option>
                                <option value="4">已发布成绩</option>
                            </select>
                        </label>--%>
                    </li>
                </ul>
    ...
    <div class="bg_fixed_phone" style="display: none">
        <div class="edit_phone_box">
           ...
           <span class="step1" style="display: none">
               <p>输入图片验证码</p>
               <label>
                   <input id="picture_code"/><img id="code_img"  onclick="this.src='/servlet/validateCodeServlet?t='+(new Date()).getTime()"/><a
...
<script type="text/javascript">
    ...
    function submitForm() {
        ...
        if( realName == ""){
            o_confirm('bg_fixed_bg_noBtn','win_box_noBtn','win_close_noBtn',"请填写您的真实姓名");
            return false;
        }
        var card = $("#id_card").val();
        if(card ==""){
            o_confirm('bg_fixed_bg_noBtn','win_box_noBtn','win_close_noBtn',"请填写您的身份证号");
            return false;
        }
    ...
    function isCardNo() {
        ...
        if(reg.test(card) === false) {
            o_confirm('bg_fixed_bg_noBtn','win_box_noBtn','win_close_noBtn',"请再核对下您的身份证号");
            return  false;
        }
    }
        ...
        if ($("#imgFile").val()) {
            ...
            $("#save_user_info").ajaxSubmit({
                ...
                success: function (data) {
                    $("#user_icon").attr("src", "http://s.sangepg.com/"+data+"?t="+(new Date()).getTime());
                    $("#userImg").val(data);
                }
            });
        }
